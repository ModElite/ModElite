...
const Summary: React.FC<Props> = ({ my_item }) => {
  const [Delivery, Setdelivery] = useState<number>(0);
  useEffect(() => {
    if (my_item.filter((item) => item.selected).length < 1) {
      Setdelivery(0);
    } else {
      Setdelivery(150);
    }
  ...
  return (
    <div>
      <div className='col-span-1 h-fit space-y-2 rounded-[20px] bg-gray-100 md:mx-6'>
        <div className='mx-6'>
        ...
        <div className='mx-6 flex'>
          ...
          <h2 className='flex-none'>{calculateTotal(0, my_item)}</h2>
        </div>
        <div className='mx-6 flex'>
        ...
        <div className='mx-6 flex py-3 text-purple1'>
          ...
          <h2 className='flex-none font-semibold'>{calculateTotal(Delivery, my_item)}</h2>
        </div>
        <hr className='mx-auto w-5/6 border-grayHr'></hr>
