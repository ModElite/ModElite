...
const OrderHistoryCard: FC<IProps> = (props) => {
  const router = useRouter();

  return (
    <div className='flex w-full flex-col gap-5 border-b p-5'>
      <div className='grid w-full grid-cols-2 place-content-center gap-2 rounded-2xl bg-[#F5F6FB] p-6 lg:grid-cols-4'>
        ...
        <div className='flex content-center items-center justify-start lg:justify-end'>
          <Button type='primary' size='large' onClick={() => router.push(`/setting/order-history/${props.orders.id}`)}>
            View detail
          </Button>
        </div>
      </div>
      ...
      <div className='flex flex-col justify-between sm:flex-row'>
        <div className='flex flex-row content-center items-center gap-2'>
          ...
          {props.status === 'DELIVERY_ON_THE_WAY' ? (
            <>
              <CiDeliveryTruck color='#60a5fa' size={24} />
              <span className='text-blue-400'>receive the product within 7 days</span>
            </>
          ) : (
            <></>
          )}
          ...
          {props.status === 'END' ? (
            <>
              ...
              <span className='text-purple2'>Success</span>
            </>
          ) : (
            <></>
          )}
